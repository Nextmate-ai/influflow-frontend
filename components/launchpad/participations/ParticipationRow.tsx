...
export const ParticipationRow: React.FC<ParticipationRowProps> = ({
  ...
  return (
    <tr className="border-b border-slate-700 hover:bg-slate-800/50 transition-colors">
      ...
      {/* 交易量 */}
      <td className="px-6 py-4">
        <span className="text-cyan-400 font-semibold">{totalVolume}</span>
      </td>

      {/* 奖励 */}
      <td className="px-6 py-4">
        <span className="text-slate-300 font-semibold">{rewards}</span>
      </td>
      ...
      {/* 状态 */}
      <td className="px-6 py-4">
        <span className={`px-3 py-1 border rounded-full text-sm font-semibold ${statusColor}`}>
          {status}
        </span>
      </td>
